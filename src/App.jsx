...
import {configured,getSite,useAsync,SiteCtx,useSite} from './lib.js'
import {Home,Catalogue,Product,About,Contact,NotFound} from './pages.jsx'
import Admin from './Admin.jsx'
function Nav(){const s=useSite(),loc=useLocation();const [top,setTop]=useState(true),[open,setOpen]=useState(false)
...
return <header className={`fixed inset-x-0 top-0 z-40 transition-all duration-500 ${ov?'py-6 text-ivory':'border-b border-sand bg-ivory/95 py-4 text-ink'}`}>
...
function Footer(){const s=useSite();return <footer className="bg-ink px-6 py-16 text-ivory md:px-12"><div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-3">
<p className="font-serif text-3xl">{s.business_name||'Saloni Furniture'}</p>
<div className="space-y-1 text-sm text-ivory/80">{s.phone&&<p>{s.phone}</p>}{s.email&&<p>{s.email}</p>}{s.address&&<p className="whitespace-pre-line">{s.address}</p>}</div>
...
<p className="mx-auto mt-12 max-w-7xl text-xs text-ivory/50">© {new Date().getFullYear()} {s.business_name||'Saloni Furniture'}</p></footer>}
